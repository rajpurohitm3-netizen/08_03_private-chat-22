...
export function Chat({ session, privateKey, initialContact, isPartnerOnline, onBack, onInitiateCall, isFriend = true, onSendFriendRequest }: ChatProps) {
  ...
  const decryptMessageContent = async (msg: any) => {
    try {
      ...
      try {
        ...
        return "⚠️ Signal mismatch: Keys don't align";
      }
    } catch (e) {
  ...
  async function fetchMessages() {
    try {
      ...
      const unviewed = validMessages.filter(m => m.receiver_id === session.user.id && !m.is_viewed) || [];
      if (unviewed.length > 0) {
        await supabase.from("messages").update({ is_viewed: true, viewed_at: new Date().toISOString() }).in("id", unviewed.map(m => m.id));
        
        const viewOnceMessages = unviewed.filter(m => m.is_view_once && m.media_type !== 'snapshot');
        for (const msg of viewOnceMessages) {
          setTimeout(() => {
            deleteMessageAfterView(msg.id);
          }, 5000);
        }
      }
    } catch (err) {
    ...
    return supabase.channel(`chat-${contactProfile.id}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "messages", filter: `receiver_id=eq.${session.user.id}` }, async (payload) => {
        if (payload.new.sender_id === contactProfile.id) {
          ...
          await supabase.from("messages").update({ is_delivered: true, delivered_at: new Date().toISOString(), is_viewed: true, viewed_at: new Date().toISOString() }).eq("id", payload.new.id);
          ...
          if (payload.new.is_view_once && payload.new.media_type !== 'snapshot') {
            setTimeout(() => {
              deleteMessageAfterView(payload.new.id);
            }, 5000);
          }
        }
      })
